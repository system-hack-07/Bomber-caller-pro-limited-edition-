...
def add_bomb_log(msg):
    bomb_status["logs"].insert(0, f"{datetime.now().strftime('%H:%M:%S')} - {msg}")
    if len(bomb_status["logs"]) > 50:
        bomb_status["logs"].pop()
...
async def run_attack(phone, duration=None):
    ...
    add_attack_log(f"🚀 Attack initiated on +91{phone}")
    if duration:
        add_attack_log(f"⏱️ Timer set for {duration} seconds")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        while attack_status["running"]:
            if duration:
                ...
                if elapsed >= duration:
                    add_attack_log(f"⏰ Timer expired after {duration}s")
            ...
            add_attack_log(f"⚡ Cycle {attack_status['cycles']} executed")
    ...
    async with aiohttp.ClientSession() as session:
        for i in range(count):
            ...
            if ok:
                ...
                bomb_status["failed"] += 1
            bomb_status["progress"] = int((i + 1) / count * 100)
            if i % 10 == 0 or i == count - 1:
                add_bomb_log(f"📡 Progress: {bomb_status['progress']}% | ✅ {bomb_status['success']} ❌ {bomb_status['failed']}")
            await asyncio.sleep(delay)

    add_bomb_log(f"✅ Done! Success: {bomb_status['success']}, Failed: {bomb_status['failed']}")
    bomb_status["running"] = False
...
<body>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div class="lg:col-span-5 glass-premium rounded-2xl p-6">
                    ...
                    <div class="space-y-4 mt-4">
                        ...
                        <div>
                            ...
                            <input id="bombCount" type="number" class="input-premium" placeholder="10" value="10" min="1">
                        </div>
                        <div>
                            <label class="text-xs text-gray-500 font-medium block mb-1.5 tracking-wider">⏱️ Delay (seconds)</label>
                        ...
                        <div class="grid grid-cols-3 gap-2 p-3 bg-black/30 rounded-xl border border-white/5">
                            <div class="text-center">
                                ...
                                <div class="text-sm font-bold text-green-400" id="bombTotal">0</div>
                            </div>
                            <div class="text-center">
                                <div class="text-[10px] text-gray-500 uppercase tracking-wider">Success</div>
                                <div class="text-sm font-bold text-emerald-400" id="bombSuccess">0</div>
                            </div>
                            <div class="text-center">
                                <div class="text-[10px] text-gray-500 uppercase tracking-wider">Failed</div>
                                <div class="text-sm font-bold text-red-400" id="bombFailed">0</div>
                            </div>
                        </div>
                        <div class="bg-black/30 rounded-xl p-3 border border-white/5">
                            <div class="flex justify-between text-xs text-gray-500">
                                <span>Progress</span>
                                <span id="bombProgressText">0%</span>
                            </div>
                ...
                <div class="lg:col-span-7 glass-premium rounded-2xl p-6">
                    ...
                    <div class="grid grid-cols-3 gap-3 mt-3">
                        ...
                        <div class="stat-premium">
                            <div class="stat-number-green" id="bombStatSuccess">0</div>
                            <div class="text-[10px] text-gray-500 uppercase tracking-wider mt-1">✅ Success</div>
                        </div>
                        <div class="stat-premium">
                            <div class="stat-number-red" id="bombStatFailed">0</div>
    ...
    <script>
        ...
        async function stopBomb() {
            try {
                await fetch('/bomb/stop', {method: 'POST'});
                document.getElementById('bombStartBtn').classList.remove('hidden');
        ...
        function pollBombStatus() {
            if (bombInterval) clearInterval(bombInterval);
            bombInterval = setInterval(async () => {
                try {
                    const res = await fetch('/bomb/status');
                    const d = await res.json();
                    
                    document.getElementById('bombTotal').textContent = d.total;
                    document.getElementById('bombSuccess').textContent = d.success;
                    document.getElementById('bombFailed').textContent = d.failed;
                    document.getElementById('bombProgress').style.width = d.progress + '%';
                    document.getElementById('bombProgressText').textContent = d.progress + '%';
                    document.getElementById('bombStatTotal').textContent = d.total;
                    document.getElementById('bombStatSuccess').textContent = d.success;
                    document.getElementById('bombStatFailed').textContent = d.failed;

                    if (d.running) {
                        document.getElementById('bombStatus').textContent = '⚡ Running...';
                        document.getElementById('bombStatus').style.color = '#34d399';
                    } else if (d.progress === 100) {
                        document.getElementById('bombStatus').textContent = '✅ Completed!';
                        document.getElementById('bombStatus').style.color = '#34d399';
                        document.getElementById('bombStartBtn').classList.remove('hidden');
                        document.getElementById('bombStopBtn').classList.add('hidden');
                        if (bombInterval) clearInterval(bombInterval);
                    ...
                    console.error('Bomb poll error:', e);
                }
            }, 800);
        }
...
@app.get("/bomb/status")
async def bomb_status_endpoint():
    return {
        "running": bomb_status["running"],
        "target": bomb_status["target"],
        "total": bomb_status["total"],
        "success": bomb_status["success"],
